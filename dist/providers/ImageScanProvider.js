...
export class GeminiImageScanner {
    mockScanner = new MockImageScanner();
    async extractFromImage(imageBuffer, mimeType) {
        ...
        try {
            ...
            if (parsed.expiry_date)
                fieldsExtracted++;
            if (parsed.mrp !== null && parsed.mrp !== undefined)
                fieldsExtracted++;
            if (parsed.strength)
                fieldsExtracted++;
